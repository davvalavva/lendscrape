...
test('validateField(key, value, schema)', (t) => {
  ...
  t.equal(validateField('uppl.avg', 2, schema), true, `[02] Returns true when given arguments in a correct way`)
  ...
  t.equal(validateField('belopp', [12, 13], schema), true, `[20] Returns true when given an array of numbers (ints only) as 2nd argument when expected by schema in 3rd arg.`)
  t.equal(validateField('årsränta', [12.73, 13], schema), true, `[21] Returns true when given an array of numbers (both ints & floats) as 2nd argument when expected array of "double" by schema in 3rd arg.`)
  t.equal(validateField('veckodagar', ['12', '13'], schema), true, `[22] Returns true when given an array of numbers as 2nd argument when expected by schema in 3rd arg.`)
  t.equal(validateField('statsränta', 1.23, schema), true, `[23] Returns true when given a float number when expected by schema in 3rd arg.`)
  t.equal(validateField('statsränta', 12, schema), true, `[24] Returns true when given an integer when expected a double by schema in 3rd arg.`)
